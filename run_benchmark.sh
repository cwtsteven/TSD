...
for i in benchmarks/*.ml; do
	NAME=$(basename $i .ml)
	ocamlfind ocamlc -package tsd,tsd.ppx -o benchmarks_build/$NAME -linkpkg benchmarks_build/$NAME.ml
	cp benchmarks/$NAME.ml benchmarks_build/inc/$NAME.ml
	sed -i "" "1s/.*/open Tsd_inc/" benchmarks_build/inc/$NAME.ml
	ocamlfind ocamlc -package tsd,tsd.ppx -o benchmarks_build/inc/$NAME -linkpkg benchmarks_build/inc/$NAME.ml
done 
