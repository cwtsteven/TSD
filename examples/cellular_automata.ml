...
let _ =
	...
	let size = int_of_string Sys.argv.(1) in 
	let total_step = int_of_string Sys.argv.(2) in 
	let world = create_automata size (Array.init size (fun i -> if i == (size-1)/2 then One else Zero)) rule54 in 
	for i = 1 to total_step do
		Array.iter (fun cell -> print_int (if (peek cell) == One then 1 else 0)) world;
		print_newline();
		step()
	done
	*)